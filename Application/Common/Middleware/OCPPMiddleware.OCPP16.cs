...
namespace OCPP.Core.Server
{
    public partial class OCPPMiddleware
    {
        ...
        /// </summary>
        private async Task Receive16(ChargePointStatus chargePointStatusStatus, HttpContext context, OCPPCoreContext dbContext)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            Application.Ocpp16.ControllerOCPP16 controller16 = new Application.Ocpp16.ControllerOCPP16(_configuration, _logFactory, chargePointStatusStatus, dbContext);
            ...
            try
            {
                while (chargePointStatusStatus.WebSocket.State == WebSocketState.Open)
                {
                    WebSocketReceiveResult result = await chargePointStatusStatus.WebSocket.ReceiveAsync(new ArraySegment<byte>(buffer), CancellationToken.None);
                    if (result != null && result.MessageType != WebSocketMessageType.Close)
                    {
                            ...
                            if (result.EndOfMessage)
                            {
                                ...
                                if (match != null && match.Groups != null && match.Groups.Count >= 3)
                                {
                                    ...
                                    if (msgIn.MessageType == "2")
                                    {
                                        ...
                                        var msgOut = await controller16.ProcessRequest(msgIn, this);

                                        // Send OCPP message with optional logging/dump
                                        await SendOcpp16Message(msgOut, logger, chargePointStatusStatus);
                                    }
                        ...
                        else
                        {
                            ...
                            await chargePointStatusStatus.WebSocket.CloseOutputAsync(WebSocketCloseStatus.MessageTooBig, string.Empty, CancellationToken.None);
                        }
                    }
                    else
                    {
                        ...
                        await chargePointStatusStatus.WebSocket.CloseOutputAsync((WebSocketCloseStatus)3001, string.Empty, CancellationToken.None);
                    }
                }
            }
            finally
        ...
        /// Waits for new OCPP V1.6 messages on the open websocket connection and delegates processing to a controller
        /// </summary>
        private async Task Reset16(ChargePointStatus chargePointStatusStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            Application.Ocpp16.ControllerOCPP16 controller16 = new Application.Ocpp16.ControllerOCPP16(_configuration, _logFactory, chargePointStatusStatus, dbContext);

            logger.LogTrace("OCPPMiddleware.OCPP16 => Reset16: ChargePoint='{0}'", chargePointStatusStatus.Id);
            ...
            {
                logger.LogInformation("OCPPMiddleware.OCPP16 => Reset16: Timeout (ChargePoint='{0}')", chargePointStatusStatus.Id);
            }
        ...
        /// </summary>
        private async Task UnlockConnector16(ChargePointStatus chargePointStatusStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext, string urlConnectorId)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            Application.Ocpp16.ControllerOCPP16 controller16 = new Application.Ocpp16.ControllerOCPP16(_configuration, _logFactory, chargePointStatusStatus, dbContext);
            ...
            logger.LogTrace("OCPPMiddleware.OCPP16 => UnlockConnector16: ChargePoint='{0}' / ConnectorId={1}", chargePointStatusStatus.Id, unlockConnectorRequest.ConnectorId);
            ...
            {
                logger.LogInformation("OCPPMiddleware.OCPP16 => UnlockConnector16: Timeout (ChargePoint='{0}' / ConnectorId={1})", chargePointStatusStatus.Id, unlockConnectorRequest.ConnectorId);
            }
        ...
        /// </summary>
        private async Task SetChargingProfile16(ChargePointStatus chargePointStatusStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext, string urlConnectorId, double power, string unit)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            Application.Ocpp16.ControllerOCPP16 controller16 = new Application.Ocpp16.ControllerOCPP16(_configuration, _logFactory, chargePointStatusStatus, dbContext);
            ...
            var setChargingProfileRequest = new SetChargingProfileRequest();
            setChargingProfileRequest.ConnectorId = connectorId;
            setChargingProfileRequest.CsChargingProfiles = new CsChargingProfiles
            {
                // Default values
                ChargingProfileId = 100,
                StackLevel = 1,
                ChargingProfilePurpose = CsChargingProfilesChargingProfilePurpose.TxDefaultProfile,
                ChargingProfileKind = CsChargingProfilesChargingProfileKind.Absolute,
            ...
            logger.LogInformation ("OCPPMiddleware.OCPP16 => SetChargingProfile16: ChargePoint='{0}' / ConnectorId={1} / Power='{2}{3}'", chargePointStatusStatus.Id, setChargingProfileRequest.ConnectorId, power, unit);
            ...
            {
                logger.LogInformation("OCPPMiddleware.OCPP16 => SetChargingProfile16: Timeout (ChargePoint='{0}' / ConnectorId={1} / Power='{2}{3}')", chargePointStatusStatus.Id, setChargingProfileRequest.ConnectorId, power, unit);
            }
        ...
        /// </summary>
        private async Task ClearChargingProfile16(ChargePointStatus chargePointStatusStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext, string urlConnectorId)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            var controller16 = new Application.Ocpp16.ControllerOCPP16(_configuration, _logFactory, chargePointStatusStatus, dbContext);

            var clearChargingProfileRequest = new ClearChargingProfileRequest
            {
                // Default values
                Id = 100,
                StackLevel = 1,
                ChargingProfilePurpose = ClearChargingProfileRequestChargingProfilePurpose.TxDefaultProfile,
                ConnectorId = 0
            };
            ...
            logger.LogTrace("OCPPMiddleware.OCPP16 => ClearChargingProfile16: ChargePoint='{0}' / ConnectorId={1}", chargePointStatusStatus.Id, clearChargingProfileRequest.ConnectorId);
            ...
            {
                logger.LogInformation("OCPPMiddleware.OCPP16 => ClearChargingProfile16: Timeout (ChargePoint='{0}' / ConnectorId={1}')", chargePointStatusStatus.Id, clearChargingProfileRequest.ConnectorId);
            }
        ...
        private async Task RemoteStartTransaction16(ChargePointStatus chargePointStatusStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext, string urlConnectorId, string idTag)
        {
            ...
            var idTagInfo = await controller16.InternalAuthorize(idTag, this, connectorId, AuthAction.StartTransaction, string.Empty, string.Empty, denyConcurrentTx);
            if (idTagInfo?.Status == IdTagInfoStatus.Accepted)
            {
                // Valid idTag => send request to charge point

                var remoteStartTransactionRequest = new RemoteStartTransactionRequest
                {
                    ConnectorId = connectorId,
                    IdTag = idTag
                };

                logger.LogInformation("OCPPMiddleware.OCPP16 => RemoteStartTransaction16: ChargePoint='{0}' / ConnectorId={1} / idTag='{2}'", chargePointStatusStatus.Id, remoteStartTransactionRequest.ConnectorId, idTag);
                ...
                {
                    logger.LogInformation("OCPPMiddleware.OCPP16 => RemoteStartTransaction16: Timeout (ChargePoint='{0}' / ConnectorId={1} / idTag='{2}')", chargePointStatusStatus.Id, remoteStartTransactionRequest.ConnectorId, idTag);
                }
            }
            else
            {
        ...
        /// </summary>
        private async Task RemoteStopTransaction16(ChargePointStatus chargePointStatusStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext, string urlConnectorId, int transactionId)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            Application.Ocpp16.ControllerOCPP16 controller16 = new Application.Ocpp16.ControllerOCPP16(_configuration, _logFactory, chargePointStatusStatus, dbContext);
            ...
            var remoteStopTransactionRequest = new RemoteStopTransactionRequest();
            remoteStopTransactionRequest.TransactionId = transactionId;

            logger.LogInformation("OCPPMiddleware.OCPP16 => RemoteStopTransaction16: ChargePoint='{0}' / ConnectorId={1} / TransactionId='{2}'", chargePointStatusStatus.Id, connectorId, transactionId);
            ...
            {
                logger.LogInformation("OCPPMiddleware.OCPP16 => RemoteStopTransaction16: Timeout (ChargePoint='{0}' / ConnectorId={1} / TransactionId='{2}')", chargePointStatusStatus.Id, connectorId, transactionId);
            }
        ...
        private async Task SendOcpp16Message(OCPPMessage msg, ILogger logger, ChargePointStatus chargePointStatusStatus)
        {
           string ocppTextMessage = null;
            ...
            await chargePointStatusStatus.WebSocket.SendAsync(new ArraySegment<byte>(binaryMessage, 0, binaryMessage.Length), WebSocketMessageType.Text, true, CancellationToken.None);
        }
    }
}
